...
3. Mantener el patron actual de una sola pagina con navegación superior `.app-nav` y botones `.nav-btn[data-target]` que sincronizan el scroll con cada `section[data-section]`.
4. Si cambias tema o navegacion, valida estados `active`, `aria-current`, la etiqueta `[data-theme-label]` y el `IntersectionObserver` en `src/scripts/main.ts`.
